...
public class AllSnapshotsGenerator {
    ...
    public static void main(String[] args) throws IOException {
        ...
        ListF<ItemProfile> profiles = new ItemProfileListGenerator().generate(1_000_000);
        ...
        for (SnapshotBuilder<? extends Snapshot> builder : snapshotBuilders) {
            ...
            File output = new File(dir.getAbsolutePath() + "/" + snapshot.getDefaultDirectory());
            output.deleteOnExit();
            output.mkdir();
            snapshot.dumpToFiles(output);
            progress.tick();
        }
        progress.summary();
    }
